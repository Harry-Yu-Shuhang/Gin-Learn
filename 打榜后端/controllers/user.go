...
func (u UserController) Register(ctx *gin.Context) {
	//接受用户名，密码和确认密码
	username := ctx.DefaultPostForm("username", "") //获取POST表单的username参数，如果没有则返回空字符串
	password := ctx.DefaultPostForm("password", "")
	confirm_password := ctx.DefaultPostForm("confirm_password", "")
	register_err_code := 4001
	...
	// }
	if username == "" {
		ReturnError(ctx, register_err_code, "用户名不能为空")
		return
	} else if password == "" {
		ReturnError(ctx, register_err_code, "密码不能为空")
		return
	} else if confirm_password == "" {
		ReturnError(ctx, register_err_code, "确认密码不能为空")
		return
	}

	if password != confirm_password {
		ReturnError(ctx, register_err_code, "密码和确认密码不一致")
		return
	}
...
func (u UserController) Login(ctx *gin.Context) {
	...
	if username == "" {
		ReturnError(ctx, login_err_code, "用户名不能为空")
		return
	} else if password == "" {
		ReturnError(ctx, login_err_code, "密码不能为空")
		return
	}

	user, _ := models.GetUserInfoByUserName(username)
	if user.ID == 0 {
		ReturnError(ctx, login_err_code, "用户名不存在")
		return
	}
	HashedPassword, _ := EnPwdCode([]byte(password)) //加密密码
	if user.Password != HashedPassword {
		ReturnError(ctx, login_err_code, "密码错误")
		return
	}
	ReturnSuccess(ctx, 0, "登录成功", user, 1)
}
